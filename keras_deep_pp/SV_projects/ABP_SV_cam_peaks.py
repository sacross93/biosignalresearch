...
def draw_10_graph(model,npydatapath,savename,peaks):
...
def max_min_index(data):
    maxidx  = np.argmax(data)
    minidx = np.argmin(data)
    return minidx,maxidx

def fold5_max_min_index(data):
    part = len(data)//5
    min1, max1 = max_min_index(data[:part])
    min2, max2 = max_min_index(data[part:part*2])
    min3, max3 = max_min_index(data[part*2:part*3])
    min4, max4 = max_min_index(data[part*3:part*4])
    min5, max5 = max_min_index(data[part*4:part*5])

    result = [min1,max1,min2+part,max2+part,min3+part*2,max3+part*2,min4+part*3,max4+part*3,min5+part*4,max5+part*4]
...
for i in range(len(validation_files)):
    ...
    for room in np.array(rooms):
        ...
        peaks = fold5_max_min_index(val_y_filtered)
        for peak in peaks:
            plt.axvline(peak, color='black', ls='--', linewidth=1)
        ...
        draw_10_graph(model,val_data,savename + '_CAM',peaks)

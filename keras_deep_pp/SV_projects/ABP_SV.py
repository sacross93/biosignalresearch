...
sql = """select file_name,VG_SV from abp_svv_generator.abp_sv_fft where date >=%s and date <=%s order by date,file_name;"""
...
model = semi_vggnet1000_stride1c()
...
for i in range(len(validation_files)):
    ...
    sql = """select distinct room_name from abp_svv_generator.abp_sv where date =%s;"""
    ...
    for room in np.array(rooms):
        ...
        sql = """select file_name,VG_SV,room_name,date,time from abp_svv_generator.abp_sv_fft where date =%s and room_name = %s order by date,file_name;"""
        ...
        plt.savefig(
           savepath + room + "_" + val_file + 'SV_test300' + '.png')
        #plt.show()
        plt.close()

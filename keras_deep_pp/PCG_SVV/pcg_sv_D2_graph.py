...
for i in range(len(validation_files)):
    ...
    for room in rooms:
        ...
        pred = pred[225:-225]
        val_y_filtered = val_y_filtered[225:-225]
        val_time = val_time[225:-225]
        ...
        graph_dir = savepath + room+val_file+'/'
        if not os.path.isdir(graph_dir):
            os.mkdir(graph_dir)

        break

        for k in range(len(val_data)//50):
            val_data[k]

        number_cnt = k*50
        data = np.load(val_data[cnt])
        data = data['arr_0']

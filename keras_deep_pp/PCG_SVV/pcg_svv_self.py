...
type = 'std'
folder_name = 'pcg_svv_004_std_2-11/'
...
sql = """select pcg_file,SVV from data_generator.PCG_SVV where date >=%s and date <=%s and type = %s order by date,pcg_file;"""
...
bestpath = savepath + filedate + 'pcg_svv_20191104.h5'
current_path = savepath + filedate + 'pcg_svv_last.h5'
...
for i in range(len(validation_files)):
    ...
    sql = """select distinct room_name from data_generator.PCG_SVV where date =%s  order by room_name;"""
    ...
    for room in rooms:
        ...
        sql = """select pcg_file,SVV,time from data_generator.PCG_SVV where date =%s and room_name =%s and type = %s order by date,pcg_file;"""

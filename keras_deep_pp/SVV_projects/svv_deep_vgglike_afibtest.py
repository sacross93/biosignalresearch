...
savepath = '/home/projects/pcg_transform/pcg_AI/deep/SVV/result/'+'200518_paper_afib_test/'
...
with open(savepath + 'all_data.csv', 'w',
    ...
    wr.writerow(['room', 'file', 'pred_svv', 'compared_svv'])
...
labels = {}
array_bae_mean = [[0] for i in range(len(validation_files)+17)]
array_bae_diff = [[0] for i in range(len(validation_files)+17)]
output_cnt = 0

validation_files = ['D-02_181114_111111.csv']
for i in range(len(validation_files)):

    val_file = validation_files[i].split('_')[1]
    ...
    sql = """select distinct room_name from abp_svv_ori3c where date =%s;"""
    ...
    for room in rooms:
        ...
        sql = """select file_name,EV_SVV,room_name,date,time from abp_svv_ori3c where date =%s and room_name = %s;"""
        ...
        with open(savepath + room + "_" + val_file + 'SVV' + '.csv', 'w',
            ...
            for j in range(len(pred)):
                wr.writerow([room,val_file,time[j],pred[j],val_y_filtered[j]])
...
c_lst = [plt.cm.rainbow(a) for a in np.linspace(0.0, 1.0, 35)]
...
plt.title("Total_Bland_Altman plot")
#plt.show()
plt.savefig(
    savepath +'ALL_BA' + '.png')
plt.close()

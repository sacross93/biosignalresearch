...
type = 'result_3ch_dataset_maxpool'

folder_name = 'result_3ch_dataset_maxpool/'
chnnal = 3
...
sql = """select file_name,EV_SVV from abp_svv where date >=%s and date <=%s  order by date,file_name;"""
...
#model.load_weights(currentpath)
...
for i in range(len(validation_files)):
    ...
    sql = """select distinct room_name from abp_svv where date =%s;"""
    ...
    for room in rooms:
        ...
        sql = """select file_name,EV_SVV,room_name,date from abp_svv where date =%s and room_name = %s;"""

...
type = 'minmax'
folder_name = 'pcg_sv_014_2018_2019_distinct/'
...
sql = """select pcg_file,SV from PCG_SV_distinct where    date >=%s and date <=%s and room_name = %s  and type = %s   order by date,pcg_file;"""
...
sql = """select pcg_file,SV from PCG_SV_distinct where  date >=%s and date <=%s  and room_name = %s and type = %s order  by room_name, date,pcg_file;"""
...
for i in range(len(validation_files)):
    ...
    sql = """select distinct room_name from PCG_SV_distinct where date =%s  and room_name = %s   order by room_name;"""
    ...
    for room in rooms:
        ...
        sql = """select pcg_file,SV,time from PCG_SV_distinct where date =%s and room_name =%s and type = %s order by room_name,date,pcg_file;"""
        ...
        pred = scipy.signal.savgol_filter(tmppred.flatten(),25,0)
        val_y = val_y.flatten()
        val_y_filtered = scipy.signal.savgol_filter(val_y,25,0)

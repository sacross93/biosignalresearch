...
from processing import cnn2d_model, get_sv_data
...
datapath = '/home/jmkim/data2/'

training_data, labels, y_time,y_data = get_sv_data([filedate],x_path=datapath)


pred_filedate = '180628'
t_data, test_labels, test_y_time,test_y_data = get_sv_data([filedate],x_path=datapath)


labels.update(test_labels)
t_data = t_data
validation_data = t_data[1::4]
test_data = t_data
test_y_data = test_y_data
test_y_time = test_y_time


partition = {'train' : training_data, 'validation' : validation_data , 'test' : test_data }
#partition = {'train' : training_data[:len(training_data)//2], 'validation' : validation_data , 'test' : test_data }
...
bestpath = '/home/projects/pcg_transform/pcg_AI/deep/SV/save_weight/test02'+filedate+'best_resnet152_predict_SV.h5'
...
for j in range(80):
    ...
    for i in range(epoch):
        ...
        history = model.fit_generator(generator=training_generator,epochs=1,validation_data=validation_generator, callbacks=[custom_hist])
...
model.save_weights('/home/projects/pcg_transform/pcg_AI/deep/SV/save_weight/test03_all'+filedate+'resnet152_predict_SV_test.h5')
#model.load_weights('/home/projects/pcg_transform/pcg_AI/deep/SV/save_weight/55'+filedate+'resnet152_predict_SV_test.h5')
...
plt.plot(val_mse[-100:], 'b', label='Validation mean_square_error')
...
corr = np.corrcoef(pred.flatten(),np.array(test_y_data))[0, 1]
plt.title(filedate+"_Mean_square_error :" + str(result[1])[:4]+ "  corr :" + str(corr)[:4])
plt.plot(pred, 'b', label="Predicted")
plt.plot(test_y_time,test_y_data, 'r', label='SV')
plt.xticks([i*(len(pred)//5-1) for i in range(6)])
plt.legend()
plt.savefig('/home/projects/pcg_transform/pcg_AI/deep/PP/result/test03_predict_PP_val.png')
plt.show()

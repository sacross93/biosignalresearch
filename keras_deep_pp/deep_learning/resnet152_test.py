...
from utils.processing import get_data, get_test_data
...
training_data, labels = get_data(validation_files)
...
partition = {'train' : training_data, 'validation' : test_data }
...
bestpath = '/home/projects/pcg_transform/pcg_AI/deep/PP/save_weight/resnet_test_predict_PP.h5'
checkpoint = keras.callbacks.ModelCheckpoint(bestpath, monitor='val_mse',verbose=1,save_best_only=True,mode='min')
callbacks_list = [checkpoint]


history = model.fit_generator(generator=training_generator,epochs=100,validation_data=validation_generator
                    ,validation_steps=20,callbacks=[checkpoint])

model.save_weights('/home/projects/pcg_transform/pcg_AI/deep/PP/save_weight/resnet_predict_PP_test.h5')

...
def save_svv_to_db(filepath, savepath, filename):
    ...
    for i in range(len(data)):
        ...
        sql = """insert into abp_testset.abp_svv_testset(date,time,room_name,file_name,EV_SVV) values (%s,%s,%s,%s,%s);"""
        curs.execute(sql, (filename[5:11],datetime.utcfromtimestamp(dt[i]+60*60*9).strftime("%y-%m-%d %H:%M:%S"), filename[:4], save, float(round(svv[i], 5))))
...
savepath = '/mnt/Data/abpsvv_testset1/'

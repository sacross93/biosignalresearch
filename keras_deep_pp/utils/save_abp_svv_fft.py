...
def save_abp_svv_10sec(filepath, file):
    ...
    return data, np.array(y_data).flatten() , fft_data
...
def save_svv_to_db(filepath, savepath, filename):
    ...
    data, svv,fft_t1 = save_abp_svv_10sec(filepath, filename)
    curs = conn.cursor()

    for i in range(len(data)):
        save = savepath+'data/' + filename[:-4] + '_' + str(i).zfill(6) + '.npy'
        savezfft = savepath+'svvzfft/' + filename[:-4] + '_' + str(i).zfill(6) + 'zfft.npy'
        savefft = savepath+'svvfft/' + filename[:-4] + '_' + str(i).zfill(6) + 'fft.npy'

        zfftdata = data[i,:,-1]
        zfftdata = np.reshape(zfftdata,(1000,1))
        #print(zfftdata.shape)
        np.save(savezfft,zfftdata)
        np.save(savefft,fft_t1[i])
        ...
        sql = """insert into abp_svv_zfft(date,room_name,file_name,EV_SVV) values (%s,%s,%s,%s);"""
        curs.execute(sql, (filename[5:11], filename[:4], save, float(round(svv[i], 5))))

        sql = """insert into svv_fft_test1(date,room_name,file_name,EV_SVV) values (%s,%s,%s,%s);"""
        curs.execute(sql, (filename[5:11], filename[:4], savezfft, float(round(svv[i], 5))))

        sql = """insert into svv_fft_test2(date,room_name,file_name,EV_SVV) values (%s,%s,%s,%s);"""
        curs.execute(sql, (filename[5:11], filename[:4], savefft, float(round(svv[i], 5))))

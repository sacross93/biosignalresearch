...
def save_abp_svv_10sec(filepath, file):
    ...
    for j in range(len(data)):
        ...
        zfft = np.abs(np.fft.fft(data[j], 1024))
    ...
    zfft_data = np.reshape(zfft_data, [len(zfft_data), 300, 1])
...
def save_svv_to_db(filepath, savepath, filename):
    ...
    for i in range(len(data)):
        ...
        sql = """insert into abp_sv_small2(date,time,room_name,file_name,VG_SV) values (%s,%s,%s,%s,%s);"""
...
savepath = '/mnt/Data/abpsvv/vgsv_sm3/'

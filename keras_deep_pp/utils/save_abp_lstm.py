...
def save_abp_svv_10sec(filepath, file):
    ...
    return zfft_data, np.array(y_data).flatten(),time
...
def save_svv_to_db(filepath, savepath, filename):
    conn = pymysql.connect(host='localhost', user='hpadmin', password='anesthesia',
    ...
    for i in range(len(data)):
        ...
        sql = """insert into abp_sv_small(date,time,room_name,file_name,VG_SV) values (%s,%s,%s,%s,%s);"""
...
savepath = '/mnt/Data/abpsvv/vgsv_sm/'

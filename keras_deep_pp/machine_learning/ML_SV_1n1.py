...
#pred = scipy.signal.savgol_filter(tmppred.flatten(),5,0)

result = model.evaluate(validation_data, val_y)


corr = np.corrcoef(pred, val_y.astype('float32'))[0, 1]
...
plt.xticks([i * (len(val_y) // 5 - 1) for i in range(6)])
plt.legend()
...
corr = np.corrcoef(pred, test_y.astype('float32'))[0, 1]

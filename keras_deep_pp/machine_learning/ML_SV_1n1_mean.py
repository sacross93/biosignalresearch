...
from utils.processing import get_ml_sv_data
...
x_data,y_data = get_ml_sv_data(filedate)
...
bestpath = '/home/projects/pcg_transform/pcg_AI/ml/PP/result/saved_weight/' + filedate + 'best_ml_predict_SV(avg).h5'
...
for j in range(300):
    ...
    for i in range(epoch):
        ...
        history = model.fit(training_data, ty)
        pred = model.predict(validation_data)
        pred = pred.flatten()
        pred_sv = []
        for i in range(5, len(pred)):
            pred_sv.append(np.average([pred[i - 4], pred[i - 3], pred[i - 2], pred[i - 1], pred[i]]))

        test_val_y = val_y[5:].astype('float32')

        mse = sklearn.metrics.mean_squared_error(test_val_y,pred_sv)
        ...
        corr = np.corrcoef(pred_sv, test_val_y)[0, 1]

        if corr > bestcorr and save_flag == 1:
            print('best corr : ' ,corr)
            bestcorr = corr
            print('saved_best_corr')
            model.save_weights(
                bestpath+'_corr')
...
plt.title(str('val') + filedate + "_Mean_square_error :" + str(mse)[:5] + "  corr :" + str(corr)[:4])
...
test_val_y = test_y[5:].astype('float32')

mse = sklearn.metrics.mean_squared_error(test_val_y, pred_sv)

corr = np.corrcoef(pred_sv, test_val_y)[0, 1]

plt.title(filedate + "_Mean_square_error :" + str(mse)[:4] + "  corr :" + str(corr)[:4])
plt.plot(pred, 'b', label="Predicted")
plt.plot(test_y, 'r', label='SV')
plt.xticks([i * (len(pred) // 5 - 1) for i in range(6)])
plt.legend()
# plt.savefig('/home/projects/pcg_transform/pcg_AI/deep/PP/result/vgg16_pp/vgg16_predict_PP_val_'+str(test01[0])+'.png')
plt.show()
...
model.load_weights(bestpath+'_corr')

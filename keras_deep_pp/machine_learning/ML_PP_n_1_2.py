...
training_files = files[2:8]
validation_files=files[8]
test_files = files[:2]

with open(save_path + 'n_1_2_pp_data_'+str(phase_flag)+'.csv', 'w',
          newline='') as corr_csv:
    wr = csv.writer(corr_csv)
    wr.writerow(['']+[test_files[i] for i in range(len(test_files))])


training_data=[]

for n in range(len(training_files)):
    #training
    filedate = training_files[n]
    if len(training_data) == 0:
        ...
        x_data, y_data = get_ml_pp_data(filedate, phase_flag)
        training_data = np.concatenate([training_data, x_data])
        ty = np.concatenate([ty, y_data])
...
bestpath = '/home/projects/pcg_transform/pcg_AI/ml/PP/result/saved_weight/n_1_2_pp_data_best_ml_predict_PP'+str(phase_flag)+'.h5'
...
plt.legend()
plt.savefig('/home/projects/pcg_transform/pcg_AI/ml/PP/result/graph/n_1_2_pp_data_'+val_filedate+'val_ml_PP_'+str(phase_flag)+'.png')
plt.close()
#plt.show()
...
for m in range(len(test_files)):
    ...
    plt.title(test_filedate+"_Mean_square_error :" + str(result[1])[:4]+ "  corr :" + str(corr)[:4])
    plt.plot(pred, 'b', label="Predicted")
    plt.plot( test_y, 'r', label='PP')
    plt.xticks([i*(len(pred)//5-1) for i in range(6)])
    plt.legend()
    plt.savefig(
        '/home/projects/pcg_transform/pcg_AI/ml/PP/result/graph/n_1_2_pp_data_' + test_filedate + 'test_ml_PP_' + str(
            phase_flag) + '.png')
    plt.close()
    """
...
with open(save_path + 'n_1_2_pp_data_' + str(phase_flag) + '.csv', 'a',
          newline='') as corr_csv:
    wr = csv.writer(corr_csv)
